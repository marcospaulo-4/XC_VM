...
echo '>' . $language::get('all_time') . '</option>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<option value="604800"';
...
echo '>' . $language::get('last_7_days') . '</option>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<option value="86400"';
...
echo '>' . $language::get('last_24_hours') . '</option>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<option value="3600"';
...
echo '>' . $language::get('last_hour') . '</option>' . "\r\n\t\t\t\t\t\t\t\t" . '</select>' . "\r\n\t\t\t\t\t\t\t" . '</div>' . "\r\n\t\t\t\t\t\t\t" . '<div class="col-md-2">' . "\r\n\t\t\t\t\t\t\t\t" . '<select id="show_entries" class="form-control" data-toggle="select2">' . "\r\n\t\t\t\t\t\t\t\t\t";
...
echo "\t\t\t\t\t\t\t\t" . '</select>' . "\r\n\t\t\t\t\t\t\t" . '</div>' . "\r\n\t\t\t\t\t\t" . '</div>' . "\r\n\t\t\t\t\t\t" . '<table id="datatable-activity" class="table table-striped table-borderless dt-responsive nowrap">' . "\r\n\t\t\t\t\t\t\t" . '<thead>' . "\r\n\t\t\t\t\t\t\t\t" . '<tr>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<th class="text-center">' . $language::get('user_id') . '</th>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<th>' . $language::get('username') . '</th>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<th class="text-center">' . $language::get('view_count') . '</th>' . "\r\n\t\t\t\t\t\t\t\t\t" . '<th class="text-center">' . $language::get('actions') . '</th>' . "\r\n\t\t\t\t\t\t\t\t" . '</tr>' . "\r\n\t\t\t\t\t\t\t" . '</thead>' . "\r\n\t\t\t\t\t\t\t" . '<tbody>' . "\r\n" . '                                ';

foreach ($rTheftDetection[$rRange] as $rRow) {
	if ($rRange == 0) {
		$rButtons = '<a href="line_activity?search=' . $rRow['username'] . '"><button type="button" class="btn btn-light waves-effect waves-light btn-xs">' . $language::get('view_logs') . '</button></a>';
	} else {
		$rDates = date($rSettings['date_format'], time() - intval($rRange)) . ' - ' . date($rSettings['date_format'], time());
		$rButtons = '<a href="line_activity?search=' . $rRow['username'] . '&dates=' . $rDates . '"><button type="button" class="btn btn-light waves-effect waves-light btn-xs">' . $language::get('view_logs') . '</button></a>';
	}
