...
class ParserFacade {
	...
	public function parse(M3UStreamInterface $stream) {
		if (NULL === $this->parser) {
			$tagDefinitions = new TagDefinitions(require __DIR__ . '/resources/tags.php');
			$this->parser = new Parser($tagDefinitions, new Config(require __DIR__ . '/resources/tagValueParsers.php'), new DataBuilder());
		}
...
class DumperFacade {
	...
	public function dump(ArrayAccess $data, M3UStreamInterface $stream) {
		if (NULL === $this->dumper) {
			$tagDefinitions = new TagDefinitions(require __DIR__ . '/resources/tags.php');
			$this->dumper = new Dumper($tagDefinitions, new Config(require __DIR__ . '/resources/tagValueDumpers.php'));
		}
...
interface M3UStreamInterface extends Iterator {
	public function add($line);
}

class FileStream extends SplFileObject implements M3UStreamInterface {
	public function add($line) {
		$this->fwrite($line . "\n");
	}
}

class TextStream extends ArrayIterator implements M3UStreamInterface {
...
class Lines implements Iterator {
	...
	public function __construct(M3UStreamInterface $stream) {
		$this->stream = $stream;
	}
