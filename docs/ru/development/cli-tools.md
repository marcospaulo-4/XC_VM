# 🛠 CLI-инструменты и миграции БД

Справочник по командной строке XC_VM, системным утилитам и системе миграций базы данных. Покрывает повседневные операции, экстренный доступ и создание новых миграций БД.
...
- Файлы миграций хранятся как `.sql` файлы в `/home/xc_vm/migrations/`.
...
- Применённые миграции отслеживаются в таблице `migrations` БД. Каждая миграция выполняется **ровно один раз** - если миграция уже применена, она пропускается.
...
- **Не смешивайте DDL и DML**, зависящие друг от друга, в одном файле. Если нужно добавить колонку и затем заполнить её, используйте два файла миграций.
