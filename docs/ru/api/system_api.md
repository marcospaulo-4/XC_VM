# 📡 Системный API (Требуется проверка методов)
...
System API обрабатывается контроллером:

```text
src/public/Controllers/Api/AdminApiController.php
```

HTTP entry-point: `/api.php` → роутится nginx на `public/index.php` с параметром `XC_API=internal`.
